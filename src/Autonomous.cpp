...
namespace dreadbot {
	...
	int ForkGrab::update() {
		if (isLiftDown()) {
			HALBot::incrTote();
			//fork->Set(0);// @todo see if this works
			...
			fork->Set(-1); // @todo see if this works
		}
		drivebase->Drive(0, 0, 0);
	...
	int Rotate::update() {
		if (driveTimer.Get() >= ROTATE_TIME) { //Rotated far enough; break
			timerActive = false;
			drivebase->Drive(0, 0, 0);
			if (HALBot::getToteCount() == 3)
	...
	int PushContainer::update() {
		...
		if (driveTimer.Get() >= pushTime) {
			timerActive = false;
			drivebase->Drive(0, 0, 0);
			intakeArms->Set(-1); // Open intake arms
			return HALBot::timerExpired;
		} else {
		...
		pusher1->Set(CONTAINER_SPIN_SPEED*pushConstant); //Push the container?
		pusher2->Set(CONTAINER_SPIN_SPEED*pushConstant);
	...
	int RotateDrive::update() {
		if (driveTimer.Get() >= (ROTATE_TIME - 1.0)) { //Rotated far enough; break
			timerActive = false;
		...
		drivebase->Drive(0, 1.0 * dir, 0.5 * rotateConstant);
		return HALBot::no_update;
	}
